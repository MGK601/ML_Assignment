...
def manhattan(vector1,vector2):
        ...
        sum = 0
        for i in range(length):
            sum += mat.fabs(vector2[i] - vector1[i])
        return sum

# To implement k-NN classifier using Euclidean distance
def knn_euclid(training_data, test, k):
    distances = [euclid(test, x) for x in training_data]
    sorted_indices = np.argsort(distances)
    k_nearest_neighbors = sorted_indices[:k]
    return k_nearest_neighbors

# To implement k-NN classifier using Manhattan distance
def knn_manhattan(training_data, test, k):
    distances = [manhattan(test, x) for x in training_data]
    sorted_indices = np.argsort(distances)
    k_nearest_neighbors = sorted_indices[:k]
    return k_nearest_neighbors
...
def predict_labels(X_train, y_train, test, k):
    k_nearest_neighbors = knn_euclid(X_train, test, k)
...
y_pred_euclidean = [predict_labels(X_train, y_train, test, k=12) for test in X_test]

# Function to predict labels for the test set using Manhattan distance
def predict_labels_manhattan(X_train, y_train, test, k):
    k_nearest_neighbors = knn_manhattan(X_train, test, k)
    k_nearest_labels = y_train[k_nearest_neighbors]
    predicted_label = np.argmax(np.bincount(k_nearest_labels.astype(int)))
    return predicted_label

# Predict labels for the test set using Manhattan distance
y_pred_manhattan = [predict_labels_manhattan(X_train, y_train, test, k=12) for test in X_test]
...
for i, test in enumerate(X_test):
    predicted_label = predict_labels(X_train, y_train, test, k=12)
...
for i, test in enumerate(X_test):
    predicted_label = predict_labels_manhattan(X_train, y_train, test, k=12)
    print(f"Test {i + 1}: Predicted class - {int(predicted_label)}")

# One-Hot Encoding
one_hot_encoded_data = onehot_code(y)
print("One-Hot Encoded Data:")
print(one_hot_encoded_data)

# Label Encoding
label_encoded_data = label_encode(y)
